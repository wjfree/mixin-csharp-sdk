...
namespace mixin_sdk_test
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Console.WriteLine("\n\n======== Test Search User  ===========\n");
            //Console.WriteLine(mixinApi.SearchUser("31243"));
            ...
            //var u = mixinApi.APPUser("Csharp" + (new Random().Next() % 100) + "test", pk);
            ...
            var websocketclient = mixinApi.WebSocketConnect().Result;

            mixinApi.StartRecive(websocketclient);

            mixinApi.SendTextMessage(websocketclient, "fd72abcd-b080-3e0e-bfea-a0b1282b4bd0", "你好！！！").Wait();

            string imgagePath = @"/Users/wjfree/Downloads/image.png";

            using (var fs = new FileStream(imgagePath, FileMode.Open))
            {
                using (var ms = new MemoryStream())
                {
                    fs.CopyTo(ms);
                    var bs = ms.ToArray();
                    var bsthumb = ImageUtilities.ResizeToFit(bs, new Size { Height = 50, Width = 50 }, false);
                    var imagemime = ImageUtilities.GetImageMimeType(bs);

                    Image image = Image.FromStream(ms);

                    mixinApi.SendImageMessage(websocketclient, "fd72abcd-b080-3e0e-bfea-a0b1282b4bd0", imgagePath, imagemime,
                            image.Width, image.Height, bs.LongLength, Convert.ToBase64String(bsthumb)).Wait();
                }
            }
        }
    }
